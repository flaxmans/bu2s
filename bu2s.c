 ...
 *  Copyright 2012,2013,2014 Samuel Melvin Flaxman. All rights reserved.
 *
 */

const char *version = "bu2s_3.4.7.5";
...
FILE *LDselSitesAvg, *LDselSitesSame, *LDselSitesDiff;
...
void calcDXY(int nSamples, FILE *DXYTS);
...
main(int argc, char *argv[])
{
    ...
    while ((ch = getopt(argc, argv, "A:a:B:b:C:c:D:d:E:e:F:f:G:g:H:I:i:K:L:l:M:m:N:O:P:p:R:S:s:T:t:U:u:V:W:w:Z:?")) != -1) {
        switch (ch) {
            case 'A':
    ...
    DXYTS = fopen("DXYtimeSeries.txt","w");
    ...
    for ( m = 0; m < nMUTATIONS; m++ ) {
        ...
        while (keepGoing) {
            ...
            if ( (totalGenerationsElapsed % TS_SAMPLING_FREQUENCY == 0) || ( (totalGenerationsElapsed+1) == (nMUTATIONS * nGENERATIONS_MAX) ) || RI_REACHED ) {
...
int calculateAlleleFrequencies(void)
{
	...
	double totalAlleles[nLOCI], allelesByPatch[nLOCI][nPATCHES], Ninv, patchWeights[nPATCHES], taln;
    double alleleFrequenciesByPatch[nLOCI][nPATCHES], AFdiff;
	double gtsum, HSsum, FST[nLOCI], HT, p, Nsinv[nPATCHES], nsnDist, nsnS, nsnLD;
	short int *spt, *stpt;
	...
	for ( locus = 0; locus < nLOCI; locus++ ) {
		if ( variable_loci[locus] ) {
			...
			if ( ((totalGenerationsElapsed % TS_SAMPLING_FREQUENCY) == 0) || ( (totalGenerationsElapsed+1) == (nMUTATIONS * nGENERATIONS_MAX) ) ) {
				if ( p <= 0.0 || p >= 1.0 )
					FST[locus] = 0.0;
				else {
					HT = 2.0 * p * (1.0 - p);
        ...
        for ( i = 0; i < nLOCI; i++ ) {
			if ( variable_loci[i] && ((locusID[i] % AL_FREQ_SUBSAMPLE) == 0) ) {
				if ( FST[i] > FST_MIN_RECORDING_THRESH ) {
                    ...
                    fprintf(AFtimeSeries, " %i %i\n", is_reversed_locus[i], locType);
...
void calculateFitnesses(double *f, double *fsum)
{
    ...
    _Bool locusTakenCareOf[nLOCI], invertFitness, useOppositeArray;
...
//*/	
	...
	if ( GAMETE_PRODUCTION_MODE == 2 ) { // bag of genes
		...
		exit(0);
		// */ // end test
        if ( (totalGenerationsElapsed % TS_SAMPLING_FREQUENCY) == 0 ) {
...
void calcExpectedME(double *fitpt, double *fitsumpt, int gatherLDvalues)
{
    ...
    if ( RECORD_LD_VALUES && (totalGenerationsElapsed > END_PERIOD_ALLOPATRY) ) {
        if ( BeginRecordingLD ) { // we already started recorded before now
            if ( (avgImmFit[0]/avgResFit[0] <= END_THRESH_FOR_LD) && migrationCount[0] && (avgImmFit[j]/avgResFit[j] <= END_THRESH_FOR_LD) && migrationCount[j] ) {
                RECORD_LD_VALUES = 0;
	...
	if ( totalGenerationsElapsed > END_PERIOD_ALLOPATRY && totalMigrants > 0 ) {
...
void printParameters(long int estRunLength)
{
    ...
    fprintf(pfpt,"TS_SAMPLING_FREQUENCY = %i;\n",TS_SAMPLING_FREQUENCY);
